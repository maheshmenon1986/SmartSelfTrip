                ...
                if st.session_state.walking_matrix and st.session_state.transit_matrix:
                    ...
                    if time_map and mode_map:
                        ...
                        if optimal_route_names and total_travel_time_seconds is not None:
                            ...
                            # This is your corrected summary display block
                            st.markdown(f"""
                            ...
                            if itinerary_map:
                                st_folium(itinerary_map, width=1400, height=600, key="main_itinerary_map_display")
                            else:
                                st.warning("Could not generate map. Check if place coordinates are available.")

                                #BUDGET SECTION
                            # First, build the dynamic HTML strings using Python loops and f-strings
                            individual_place_fees_html = ""
                            for place_data in st.session_state.places_results:
                                # Ensure 'entry_fee' is already calculated and attached to place_data
                                # by the calculation logic above this output section.
                                individual_place_fees_html += f"<p>Place: {place_data.get('name', 'Unnamed Place')}<br>Expected Spending / Fees: ${place_data.get('entry_fee', 0.0):,.2f}</p>"

                            # Then, combine all parts into one markdown string
                            st.markdown(f"""
                                <div class="budget_splitin">
                                    <h1>Individual Place Entry Fee Breakdown</h1>
                                    {individual_place_fees_html}
                                    <h1>Estimated Costs Summary</h1>
                                    <p>Total Trip Expense: ${total_expense:,.2f}</p>
                                    <p>Total Transit Estimated Expense: ${total_transit_cost:,.2f}</p>
                                    <p>Total Place Estimated Expense: ${total_estimated_place_fees:,.2f}</p>
                                </div>
                                """, unsafe_allow_html=True)
                        else:
